...
//     vector<vector<string>>tmp=vector<vector<string>>(m.numCols(), vector<string>(m.numRows()));
...
//         // printf("numCols, m.numCols, %d %d\n", numCols(), data_frame.numCols());
//         // assert((numCols()==0||data_frame.numCols()==0||numCols()==data_frame.numCols())&&"Both matrices must have same number of columns to be concatenable along axis 1.");

//         // DataFrame temp = DataFrame(getRows()+data_frame.getRows(), max(numCols(), data_frame.numCols()));
...
//         //     for(size_t j=0; j<data_frame.numCols(); j++){
